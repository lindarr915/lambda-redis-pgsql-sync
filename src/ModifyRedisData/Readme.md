sss# AWS Lambda Empty Docker Image Function Project
...
WORKDIR /src
COPY ["ModifyRedisData.csproj", "ModifyRedisData/"]
RUN dotnet restore "ModifyRedisData/ModifyRedisData.csproj"

WORKDIR "/src/ModifyRedisData"
COPY . .
RUN dotnet build "ModifyRedisData.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "ModifyRedisData.csproj" \
...
Execute unit tests
```
cd "ModifyRedisData/test/ModifyRedisData.Tests"
dotnet test
```

Deploy function to AWS Lambda
```
cd "ModifyRedisData/src/ModifyRedisData"
